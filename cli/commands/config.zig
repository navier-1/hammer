...
fn config(args: [][:0]u8) anyerror!ConfigParams {
    ...
    if (cml) |_| {
        ...
        const target = ".configure"; // TODO: read the configuration dir from (heh) configuration
        ...
        const reserved_dir_path = try std.fmt.allocPrintZ(allocator, "{s}/{s}", .{project_dir, ".reserved"});

        const reserved_dir: ?std.fs.Dir = cwd.openDir(reserved_dir_path, .{}) catch |err| switch (err) {
            error.FileNotFound => null,
            else => return err,
        };
