...
# Load and parse map file for specific symbols; it needs help from the binary to convert virtual addresses to offsets
def parse_map_file(map_file_path, symbols, binary):
    symbol_info = {}
    last_address = None
    last_symbol = None
    ...
    with open(map_file_path, 'r') as f:
        ...
        for line in lines:
            try:
                parts = line.split()
                vma, size, symbol_name = int(parts[0], 16), int(parts[2], 16), parts[4]
                if symbol_name in symbols:
                    offset = binary.virtual_address_to_offset(vma) - text_offset
                    symbol_info[symbol_name] = (offset, size, vma)
            except:
                pass
...
def patch_placeholder_with_size(binary, stub_address, function_size, stub_size):
    # For Linux, match placeholder in a range, since the compiler changes its value a little bit
    ...
    for i in range(stub_offset, stub_offset + stub_size - 3):
        actual_placeholder = int.from_bytes(content[i:i+4], 'little')
        if (placeholder - 32 < actual_placeholder < placeholder + 32):
            delta = placeholder - actual_placeholder
            function_size = function_size - delta
            ...
            print(f"Patched {hex(actual_placeholder)} at offset {i} with function size (delta applied).")
            # Do not break, keep patching all placeholders found
            # break
            patched = True
    else:
        if not patched:
...
if __name__ == "__main__":
    ...
    symbol_data = parse_map_file(map_file_path, target_symbols, binary)
    print(symbol_data)
